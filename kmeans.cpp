...
bool point::operator!=(const point& p) { return !(*this == p); }
...
void point::operator+=(const point& p) {
    for (int i = 0; i < point::d; i++) this->coords[i] += p.coords[i];
    return;
}
...
double point::sq_dist(const point& q) const {
    ...
    for (int i = 0; i < d; ++i) {
        dist +=
            (this->coords[i] - q.coords[i]) * (this->coords[i] - q.coords[i]);
    }
    return dist;
}

double point::dist(point& q) { return sqrt(this->sq_dist(q)); }

double cloud::get_sihouette() {
    double sum = 0;
    for (int i = 0; i < n; i++) {
        sum += (get_outcluster_coeff(i) - get_incluster_coeff(i)) /
               ((get_outcluster_coeff(i) > get_incluster_coeff(i))
                    ? get_outcluster_coeff(i)
                    : get_incluster_coeff(i));
    }
    return sum / (double)n;
}
...
void cloud::init_plusplus() {
    ...
    sq_dist.resize(n);
    for (int i = 0; i < n; i++) sq_dist[i] = points[i].sq_dist(points[c]);
...
void cloud::set_voronoi_labels() {
    for (int i = 0; i < n; i++) {
        for (int center = 0; center < k; center++) {
            if (points[i].dist(centers[points[i].label]) >
                points[i].dist(centers[center]))
...
void cloud::set_centroid_centers() {
    ...
    //
    for (int i = 0; i < n; i++) {
        //		std::cout << "=== "<<i <<" with lbl: " <<points[i].label << "-" <<
        //std::endl; 		sum[points[i].label].print();
        sum[points[i].label] += points[i];
...
void test_intracluster_variance() {
    ...
    assert(std::fabs(twoclusters.intracluster_variance() - dist_twoclusters) <
           eps);
...
void test_init_forgy() {
    ...
    for (int k = 0; k < K; k++) {
        ...
        if (threepoints.get_center(0).coords[0] == 1.0) cnt++;
    }
...
void test_init_plusplus() {
    ...
    for (int k = 0; k < K; k++) {
        ...
        if (twoclusters.get_center(1).coords[0] == 1.0) cnt++;
    }
...
void test_init_random_partition() {
    ...
    for (int k = 0; k < K; k++) {
        ...
        if (threepoints.get_point(2).label == 1) cnt++;
    }
    assert(std::fabs(cnt / (double)K - prob_threepoints) < delta);
...
int test_TD3(int argc, char** argv) {
    ...
    if (argc < 5 || argc > 6) {
        std::cerr << "Usage: " << argv[0]
                  << " csv nb_clusters x_column y_column [ test ]" << std::endl;
        std::exit(1);
    }
    ...
    std::cout << "Intracluster variance after k-means: "
              << c.intracluster_variance() << std::endl;
    std::cout << "Sihouette value of k=" << k << " is: " << c.get_sihouette()
              << std::endl;
    ...
    // int gtk_argc = 1;
    // Glib::RefPtr<Gtk::Application> app = Gtk::Application::create(gtk_argc,
    // argv, "inf442.td3");
...
//        cr->arc((c->get_point(i).coords[x_column] - x_min) * width / (x_max -
//        x_min),
//                (c->get_point(i).coords[y_column] - y_min) * height / (y_max -
//                y_min), 10.0, 0.0, 2.0 * M_PI); // full circle
//
//        // choose color depending on label
//        switch (c->get_point(i).label) {
//            case 0:
//                cr->set_source_rgba(1.0, 0.0, 0.0, 0.6); // red, partially
//                translucent break;
//
//            case 1:
//                cr->set_source_rgba(0.0, 0.0, 0.8, 0.6); // 0.8 blue,
//                partially translucent break;
//
//            case 2:
//                cr->set_source_rgba(0.0, 1.0, 0.0, 0.6); // green, partially
//                translucent break;
//
//            default:
