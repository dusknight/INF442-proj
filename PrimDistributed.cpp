...
double PrimPaSolver::CalcMST()
{
	...
	if(world_rank!= world_size-1) //分配给每个进程指定的处理顶点
	{
	...
	while(count<taille) //控制循环
	{
        ...
        int minor = INF;
        int in_which_process = world_rank;

        for (int j = number_per_process[0]; j <= number_per_process[1]; j++) //找到当前分支中未加入的节点中距离当前已得森林中最近的点
        {
        ...
        int sub_data[3] = {cur, minor, in_which_process};
		...
		if(world_rank==0) //从收进来的数据中找出到已有分支最近的点
		{
            int cur_0 = 1;
            int minor_0 = INF;
            int in_which_proc = 1;
   ...
   //             cout << "Not Connected" << endl;
      ...
      {
          int edge_number = 0;
          if (graph->getEdgeCost(sub_data[0], closest[sub_data[0]]) > graph->getEdgeCost(closest[sub_data[0]], sub_data[0])) // 选择加入的边
          {
