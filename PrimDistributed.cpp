...
void PrimPaSolver::ReadFile(string filename) { graph->ReadFile(filename); }
...
double PrimPaSolver::CalcMST() {
    ...
    if (world_rank != world_size - 1) {
        ...
        number_per_process[1] =
            number_per_process[0] + (int)(taille / world_size) - 1;
    }
    if (world_rank == world_size - 1) {
        number_per_process[0] = world_rank * (int)(taille / world_size) + 1;
    ...
    while (count < taille) {
        ...
        for (int j = number_per_process[0]; j <= number_per_process[1];
             j++)  // Find the cheapest vertex for each process
        {
        ...
        int sub_data[3] = {
            cur, minor,
            in_which_process};  // Prepare to send information to main process
        MPI_Barrier(MPI_COMM_WORLD);
        int* sub_datas = NULL;
        if (world_rank == 0) {
            sub_datas = (int*)malloc(sizeof(int) * world_size * 3);
        }
        MPI_Gather(sub_data, 3, MPI_INT, sub_datas, 3, MPI_INT, 0,
                   MPI_COMM_WORLD);  // Main Process Collect Information

        MPI_Barrier(MPI_COMM_WORLD);

        if (world_rank ==
            0)  // Main Process find the cheapest from every process
        {
        ...
        MPI_Bcast(sub_data, 3, MPI_INT, 0,
                  MPI_COMM_WORLD);  // BroadCast the vertex founded
        ...
        MPI_Barrier(MPI_COMM_WORLD);
        // cout << "talaizhao " << sub_data[2] << " WORLD " << world_rank <<
        // endl;

        // if (world_rank == sub_data[2]) //The corresponding process update
        //{
        //	if (graph->getEdgeCost(sub_data[0], closest[sub_data[0]])  >
        //graph->getEdgeCost(closest[sub_data[0]], sub_data[0])) // Choose the
        //direction to add
        //	{
        //		edge_number = graph->findEdge(closest[sub_data[0]],
        //sub_data[0]);
        //		//cout << "AAA" << endl;
        //	}
        //	else
        //	{
        //		edge_number = graph->findEdge(sub_data[0],
        //closest[sub_data[0]]);
        //		//cout << "BBB" << endl;
        //	}
        //
        ...
        //
        // MPI_Barrier(MPI_COMM_WORLD);
        // MPI_Bcast(&edge_number, 1, MPI_INT, sub_data[2], MPI_COMM_WORLD);
        // //Broadcast the index founded of the edge
        ////cout << "ZHEGE edge " << edge_number << endl;
        // MSTedges.push_back(edge_number);
        ans += sub_data[1];
        MPI_Barrier(MPI_COMM_WORLD);

        for (int j = number_per_process[0]; j < number_per_process[1] + 1;
             j++)  // Update Cost
        {
            if (!visited[j] &&
                costs[j] > min(graph->getEdgeCost(j, sub_data[0]),
                               graph->getEdgeCost(sub_data[0], j))) {
                costs[j] = min(graph->getEdgeCost(j, sub_data[0]),
                               graph->getEdgeCost(sub_data[0], j));
