...
bool Edge::cmp(Edge a, Edge b) { return a.cost < b.cost; }
...
GraphInEdge::GraphInEdge() : Graph() { graphName = ""; }
...
int GraphInEdge::findEdge(int u, int v) {
    ...
    while (i < l) {
        if (edges[i].u == u) {
            if (edges[i].v == v) {
                // std::cout << "The edge is located in "<< i <<"th position in
                // edges"<<std::endl;
                return i;
            }
        }
...
double GraphInEdge::getEdgeCost(int u, int v) {
    ...
    //		{
    //			//std::cout << "The edge is located in " << i << "th position in
    //edges"<<std::endl; 			return edges[i].cost;
    //		}
    //	}
    //	i += 1;
    //}
