...
int GHSNode::assign_id(int id_rank, int num_process) { return id_rank; }
...
void GHSNode::MsgHandler(GHSmsg msg, int from_edge) {
    switch (msg.type) {
        case MsgType::CONNECT:
            ...
            break;
        case MsgType::INIT:
            RespInit(msg.arg1, msg.argf, (GHSNode::NodeState)msg.arg2,
                     from_edge);  // L, F, S
            break;
        case MsgType::TEST:
        ...
        case MsgType::CHANGE_CORE:
            ...
            break;
        default:
            std::cerr << "[Error]: unimplemented message type " << msg.type
                      << std::endl;
    }
    return;
}
    ...
    */
    ...
    for (auto ip = adj_out_edges.begin(); ip != adj_out_edges.end(); ip++) {
        if (ip->second.state == GHSEdge::EdgeState::BASIC)
            if (ip->second.cost < weight ||
                (ip->second.cost == weight && ip->first < best_edge)) {
    ...
    //        if (adj_out_edges[ie].cost < weight || adj_out_edges[ie].cost ==
    //        weight && ie < best_edge) {
...
int GHSNode::find_test_edge() {
    ...
    */
    ...
    for (auto ip = adj_out_edges.begin(); ip != adj_out_edges.end(); ip++) {
        if (ip->second.state == GHSEdge::EdgeState::BASIC)
            if (ip->second.cost < weight ||
                (ip->second.cost == weight && ip->first < test_edge)) {
                weight = ip->second.cost;
                test_edge = ip->first;
...
void GHSNode::RespInit(
    int L, double F, GHSNode::NodeState S,
    int from_node)  // ============= HERE LIES THE LAST PROBLEM
{
    ...
    for (auto ip = adj_out_edges.begin(); ip != adj_out_edges.end(); ip++) {
        // if (ip->first != from_node && ip->second.state ==
        // GHSEdge::EdgeState::BRANCH) {
        ...
        if (ip->first == from_node ||
            ip->second.state != GHSEdge::EdgeState::BRANCH)
            continue;
        // send INIT (L, F, S) on edge i
        machine->send_msg(GHSmsg(MsgType::INIT, L, F, S, ip->first, id));
        if (S == FIND) find_count++;
...
void GHSNode::RespConnect(int level,
                          int from_node) {  // checked ok (process_connect)
    ...
    if (level < LN) {  // absorb
        ...
        if (adj_out_edges[from_node].state ==
            GHSEdge::EdgeState::BASIC) {  // delay
            ...
            machine->send_msg(GHSmsg(MsgType::INIT, LN + 1,
                                     adj_out_edges[from_node].cost, FIND,
                                     from_node, id));
        }
    }
}
...
{
    test_edge = -1;
    if (Edge::cmp(adj_out_edges[from_node],
                  adj_out_edges[best_edge]))  // cost: a < b
        best_edge = from_node;
...
void GHSNode::Report()  // checked
{
    if (find_count == 0 && test_edge < 0) {  // THIS LINE IS IMPORTANT    ######
        SN = FOUND;
        machine->send_msg(GHSmsg(MsgType::REPORT, adj_out_edges[best_edge].cost,
                                 in_branch, id));
    }
}
...
void GHSNode::RespReport(double w, int from_edge)  // TODO: check (OK?
{
    if (from_edge != in_branch) {
        find_count--;
        if (w < adj_out_edges[best_edge].cost)
            best_edge = from_edge;  // update new best edge
        Report();
    } else {
        if (SN == FIND)  // place the received message at the end of queue
        {
            machine->emplace_recv_queue(
                GHSmsg(MsgType::REPORT, w, id, from_edge));
        } else if (w > adj_out_edges[best_edge].cost) {
            if (PRINT_MSG)
                cout << "[INFO] "
                     << "REPORT to CHANGECORE [Node  " << id << "];" << endl;
            ChangeCore();
        }
        // The algo is FININSHED
        else if (w == DBL_MAX &&
                 best_edge <
                     0)  // TODO: conditions not finished // TODO: check it!
        {                // HALT
            finished = true;
            /*
            We need to send notify the other nodes on the chaine that the
            algorithm has terminated. So, here, we send REPORT message with
            maximum weights.
            */
            for (auto ip = adj_out_edges.begin(); ip != adj_out_edges.end();
                 ip++) {
                if (ip->second.state == GHSEdge::EdgeState::BRANCH &&
                    ip->first != in_branch) {
                    if (PRINT_MSG)
                        cout << "[INFO] "
                             << "Send FINISH to [Node  " << ip->first
                             << "], from " << id << ";" << endl;
                    machine->send_msg(
                        GHSmsg(MsgType::REPORT, DBL_MAX, ip->first, id));
                }
            }
        }
    }
...
void GHSNode::RespChangeCore() { ChangeCore(); }

bool GHSNode::isFinished() { return finished; }
...
bool GHSMPI::ask_all_processes_if_finished() {
    if (!ask_all_nodes_if_finished())
        return false;  // this process is still working
...
void GHSMPI::send_msg(GHSmsg msg) { to_send.emplace(msg); }

void GHSMPI::emplace_recv_queue(GHSmsg msg) { recved_later.emplace(msg); }
...
void GHSMPI::init(string fname) {
    ...
    for (auto v_iter = vertices_id.begin(); v_iter != vertices_id.end();
         v_iter++) {  // init GHSNodes
...
void GHSMPI::msg_handler(GHSmsg msg) {
    ...
    switch (msg.type) {
        case MsgType::CONNECT:
            ...
            break;
        case MsgType::INIT:
            nodes[dest_vid].RespInit(msg.arg1, msg.argf,
                                     (GHSNode::NodeState)msg.arg2,
                                     src_vid);  // LN, FN, SN
            break;
        case MsgType::TEST:
...
void GHSMPI::finalize() {
    ...
    MPI_Finalize();
    cout << "--- Node rank " << rank << " (out of " << count_machine
         << ") is finished." << endl;
    system("pause");
}
...
//    for (int i = 0; i < count_machine; i++)
//    send_counts.push_back(send_buffers[i].size());
...
void GHSMPI::exchange_with_all_machine() {
    ...
    if (PRINT_MPI_LOG_DETAIL) {
        cout << "[INFO] in " << rank << ": count_machine : " << count_machine
             << endl;  //  << "size calculated: s=" << ssize << ", r=" << rsize
                       //  << endl;
        cout << "[INFO] in " << rank
             << ": BEFORE SEND COUNT : ";  //  << "size calculated: s=" << ssize
                                           //  << ", r=" << rsize << endl;
    ...
    MPI_Allgather(&working, 1, MPI_INT, process_working_status.data(), 1,
                  MPI_INT, comm.comm);
    if (PRINT_MPI_LOG_DETAIL) {
    ...
    if (PRINT_MPI_LOG_DETAIL)
        cout << "[INFO] in " << rank << ", "
             << "size calculated: s=" << ssize << ", r=" << rsize << endl;
...
#ifndef NONMPI
    int test_val = MPI_Alltoallv(sbuffer.data(), send_counts.data(),
                                 this->sdispl, comm.getType(),
                                 this->recv_buffers.data(), recv_counts.data(),
                                 this->rdispl, comm.getType(), comm.comm);
    if (PRINT_MPI_LOG_DETAIL)
        cout << "[MPI] in " << rank << ", " << test_val << endl;
#endif  // !NONMPI
    ...
    sbuffer.clear();
    for (auto iv = send_buffers.begin(); iv != send_buffers.end(); iv++)
        iv->second.clear();
}
