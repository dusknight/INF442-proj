...
struct GHSEdge : public Edge {
    ...
    GHSEdge(Edge e, EdgeState es_) : Edge(e) { state = es_; }
    GHSEdge(Edge e) : Edge(e) { state = BASIC; }
    GHSEdge(const GHSEdge& e) : Edge(e) { state = e.state; }
    GHSEdge() {
        state = BASIC;
        u = -1;
        v = -1;
...
struct GHSmsg {
    ...
    GHSmsg() {
        type = ERR_TYPE;
        arg1 = arg2 = arg3 = argf = dest_vid = src_vid = -1;
    }
    GHSmsg(MsgType mt, int _dest, int _src)
        : type(mt), dest_vid(_dest), src_vid(_src){};
    GHSmsg(MsgType mt, int _arg1, int _dest, int _src)
        : type(mt), arg1(_arg1), dest_vid(_dest), src_vid(_src){};
    GHSmsg(MsgType mt, int _arg1, int _arg2, int _dest, int _src)
        : type(mt), arg1(_arg1), arg2(_arg2), dest_vid(_dest), src_vid(_src){};
    GHSmsg(MsgType mt, int _arg1, int _arg2, int _arg3, int _dest, int _src)
        : type(mt),
          arg1(_arg1),
          arg2(_arg2),
          arg3(_arg3),
          dest_vid(_dest),
          src_vid(_src){};
    GHSmsg(MsgType mt, int _arg1, double _argf, int _arg2, int _dest, int _src)
        : type(mt),
          arg1(_arg1),
          arg2(_arg2),
          argf(_argf),
          dest_vid(_dest),
          src_vid(_src){};
    GHSmsg(MsgType mt, double _f, int _dest, int _src)
        : type(mt), argf(_f), dest_vid(_dest), src_vid(_src){};
    GHSmsg(MsgType mt, int _arg1, double _argf, int _dest, int _src)
        : type(mt), arg1(_arg1), argf(_argf), dest_vid(_dest), src_vid(_src){};
    GHSmsg(const GHSmsg& msg)
        : type(msg.type),
          arg1(msg.arg1),
          arg2(msg.arg2),
          arg3(msg.arg3),
          argf(msg.argf),
          dest_vid(msg.dest_vid),
          src_vid(msg.src_vid){};
};
...
class GHScomm {
   ...
   public:
    ...
    void commitType(MPI_User_function* f) {
        ...
        MPI_Aint disp[7] = {
            offsetof(GHSmsg, type),   offsetof(GHSmsg, arg1),
            offsetof(GHSmsg, arg2),   offsetof(GHSmsg, arg3),
            offsetof(GHSmsg, argf),   offsetof(GHSmsg, dest_vid),
        ...
        const MPI_Datatype types[] = {MPI_INT,    MPI_INT, MPI_INT, MPI_INT,
                                      MPI_DOUBLE, MPI_INT, MPI_INT};

        MPI_Type_create_struct(sizeof(types) / sizeof(*types), blocklen, disp,
                               types, &GHSmsgType);
        MPI_Type_commit(&GHSmsgType);

        if (f != nullptr) MPI_Op_create(f, 1, &op);
    }
    MPI_Datatype getType() { return GHSmsgType; }
    ...
    GHSmsg recv(int& from_m_node) {
        MPI_Status status;
        GHSmsg msg;
        MPI_Recv(&msg, 1, GHSmsgType, MPI_ANY_SOURCE, MPI_ANY_TAG, comm,
                 &status);
        return msg;
    }
...
class GHSNode : public GraphInEdge {
    /**
     * See
     *https://raw.github.com/arjungmenon/Distributed-Graph-Algorithms/master/Minimum-Spanning-Tree/papers/GHS_enhanced.pdf
...
class GHSMPI : public GraphInEdge {
   ...
   public:
    ...
    int assign_vertice_to_machine(
        int v_id);  // values will be added to vert2machine
