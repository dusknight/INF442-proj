...
double PrimSolver::CalcMST()
{
    ...
    for (int i = 1; i < taille; i++) //Traverse Vertex
    {
        ...
        double minor = INF;
        for (int j = 1; j < taille + 1; j++) //Find the cheapest vertex not visited
        {
...
double BoruvkaSolver::CalcMST()
{
    ...
    while (NumCompos > 2) //When the Number of Components >1
    {
        ...
        for (int i = 0; i < graph->getVertSize() + 1; i++) { cheapest[i] = -1; }//Initialize the cheapest edge found for each component

        for (int i = 0; i < TailleEdges; i++) //traverse edges
        {
            int faU = findFather(father, graph->edges[i].u);           //Check the component
        ...
        for (int i = 0; i < NumCompos; i++)
        {
            ...
            if (cur != 0)
            {
                if (cheapest[cur] != -1)
                {
                    ...
                    father[Indu] = Indv;
                    if (!is_element_in_vector(edges_mst, cheapest[cur])) // Avoid adding repeated edge
                    {
